import cloudinary, { uploadOptimizedImage, getOptimizedImageUrl, optimizeExistingImage, generateImageVariants, getOptimizationSettings } from '../../../utils/cloudinary.js';
import { asyncHandler } from '../../../utils/asyncHandler.js';
import { sendPushNotificationToUser, createMultilingualNotification } from '../../../utils/pushNotifications.js';
import fs from 'fs';
...
export const uploadImage = asyncHandler(async (req, res) => {
  try {
    ...
    const { title, description, tags, category, applyWatermark, optimizationLevel = 'medium' } = req.body;
    ...
    const getOptimizationSettings = (level) => {
      switch (level) {
        ...
        case 'high':
          return { quality: 'auto:best', fetch_format: 'auto', flags: 'progressive', eager_async: true };
        case 'medium':
        default:
...
export const getAlbumImages = asyncHandler(async (req, res) => {
  try {
    ...
    const [images, totalCount] = await Promise.all([
      imageModel.find({ user: userId, album: albumName })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(parseInt(limit)),
...
export const optimizeAllUserImages = asyncHandler(async (req, res) => {
  ...
  process.nextTick(async () => {
    try {
      ...
      console.log(`Optimization job ${jobId} completed: ${successCount} succeeded, ${failureCount} failed`);
